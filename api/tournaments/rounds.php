...
class TournamentManager {
    ...
    /**
     * Initialize session-based bye tracking for a tournament
     */
    private function initializeByeTracking() {
        ...
        if (!isset($_SESSION['bye_tracking'][$this->tournamentId])) {
            $_SESSION['bye_tracking'][$this->tournamentId] = [
                'byes_awarded' => [], // player_id => round_number
                'rounds_completed' => [] // round_number => bool
            ];
        }
    }
     ...
     */
    public function hasPlayerReceivedBye($playerId): bool {
        $this->initializeByeTracking();
        return isset($_SESSION['bye_tracking'][$this->tournamentId]['byes_awarded'][$playerId]);
    }

    /**
     * Record that a player received a bye in a specific round
     */
    private function recordPlayerBye($playerId, $roundNumber): void {
        $this->initializeByeTracking();
        $_SESSION['bye_tracking'][$this->tournamentId]['byes_awarded'][$playerId] = $roundNumber;
    }

    /**
     * Check if a round has been completed (all matches done)
     */
    private function isRoundCompleted($roundNumber): bool {
        $this->initializeByeTracking();
        return isset($_SESSION['bye_tracking'][$this->tournamentId]['rounds_completed'][$roundNumber]);
    }

    /**
     * Mark a round as completed
     */
    private function markRoundCompleted($roundNumber): void {
        $this->initializeByeTracking();
        $_SESSION['bye_tracking'][$this->tournamentId]['rounds_completed'][$roundNumber] = true;
    }

    /**
     * Record bye information in the round itself (more reliable than session)
     */
    private function recordRoundBye($roundId, $playerId): void {
        // First get current bye players
        ...
        // Update with new bye players list
        $newByeList = implode(',', array_filter($currentByes, function($id) {
            return !empty($id);
        }));
        
        $sql = "UPDATE tournament_rounds SET bye_players = ? WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("si", $newByeList, $roundId);
        $stmt->execute();
    }

    /**
     * Get all bye data for the current tournament
     */
    public function getByeData(): array {
        $this->initializeByeTracking();
    ...
    private function persistSwissPairings(int $roundId, array $pairings): void {
        // Clean up any existing scheduled matches for this round to avoid duplicates.
        $sql = "DELETE FROM tournament_matches WHERE round_id = ? AND status = 'scheduled'";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("i", $roundId);
        $stmt->execute();

        // Remove database-based bye tracking (no longer using tournament_byes table)
        // Byes are now tracked in session memory only
        ...
        foreach ($pairings as $pair) {
            ...
            // Create bye matches in database so they're visible to all users
            if (!empty($pair['is_bye'])) {
                ...
                // Record bye in session memory
                $roundNumber = $this->getRoundNumberById($roundId);
                $this->recordPlayerBye($playerOne, $roundNumber);
                
                // Create bye match in database for visibility
                $sql = "INSERT INTO tournament_matches (tournament_id, round_id, match_number, player1_id, player2_id, status, winner_id) 
                        VALUES (?, ?, ?, ?, ?, 'completed', ?)";
                $stmt = $this->conn->prepare($sql);
                $player2Null = null;
                $stmt->bind_param("iiisss", $this->tournamentId, $roundId, $matchNumber, $playerOne, $player2Null, $playerOne);
                $stmt->execute();
                
                // Store bye information in round for easy access
                $this->recordRoundBye($roundId, $playerOne);
    ...
    private function getRoundNumberById(int $roundId): int {
        $sql = "SELECT round_number FROM tournament_rounds WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("i", $roundId);
        $stmt->execute();
        $result = $stmt->get_result()->fetch_assoc();
        return $result ? (int)$result['round_number'] : 0;
    }
...
class SwissPairing {
    ...
    private function pairFirstRound(array $meta): array {
        ...
        // For odd player counts, exclude the last player from fold pairing
        // They will receive the BYE
        $pairableCount = $isOdd ? $count - 1 : $count;
        $half = intdiv($pairableCount, 2);
        ...
        if ($isOdd) {
            ...
            if (($pairings[count($pairings) - 1]['is_bye'] ?? false) === false) {
                // Every player has already received a bye; assign to last player.
                $pairings[] = [
                    'p1' => $meta[$count - 1]['id'],
                    'p2' => null,
                    'is_bye' => true,
    ...
    private function pairByScoreGroups(array $meta): array {
        // BYE RULE: Byes are only assigned in first round
        // For rounds > 1, we float players instead of giving byes
        $groups = [];
        ...
        foreach ($scores as $index => $score) {
            ...
            if (count($group) % 2 === 1) {
                // Float the lowest-ranked player to the next score group instead of giving a bye
        ...
        // If we still have a floater after processing all groups, pair them with the lowest-ranked player
        // This should rarely happen but handles edge cases
        if ($floater !== null && !empty($pairings)) {
            // Find the lowest-ranked player and pair with floater
            $lastPairing = array_pop($pairings);
            $pairings[] = [
                'p1' => $floater['id'],
                'p2' => $lastPairing['p2'] ?? $lastPairing['p1'],
                'is_bye' => false,
            ];
        }
    ...
    switch ($action) {
        ...
        case 'getState':
            ...
            try {
                ...
                // Add bye data to response
                $byeData = $mgr->getByeData();

                $response = ['success' => true, 'rounds' => $roundEntries, 'byes' => $byeData];
            } catch (Exception $e) {
